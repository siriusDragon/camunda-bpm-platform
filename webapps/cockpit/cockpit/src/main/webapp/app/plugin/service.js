...
define([ "angular", "jquery"], function(angular, $) {
  ...
  function PluginsProvider() {
    ...
    /**
     * Initializes plugin map to replace app:// references 
     * with the actual app location
     * 
    ...
    function initializePlugins(map, uri) {
      angular.forEach(map, function(plugins) {
        angular.forEach(plugins, function(plugin) {
          if (plugin.url) {
            var url = plugin.url, 
                indexOfApp = url.indexOf(APP_REFERENCE);
            
            // starts with APP_REFERENCE?
            if (indexOfApp === 0) {
              plugin.url = uri.appUri(url.substring(APP_REFERENCE.length, url.length));
            }
          }
        });
      });
